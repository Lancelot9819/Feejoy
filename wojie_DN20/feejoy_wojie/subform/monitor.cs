...
namespace feejoy_wojie.subform
{
    ...
    public partial class monitor : DevExpress.XtraEditors.XtraForm
    {
        ...
        private int PLC_connect_DN20_2()
        {
            try
            {
                DN20_2.ActLogicalStationNumber = 21;
                DN20_2.ActPassword = "";
                returncode = DN20_2.Open();
            }
            catch
            {
            }
        ...
        private int PLC_disconnect_DN20_2()
        {
            try
            {
                returncode = DN20_2.Close();
            }
            catch
            {
            }
        ...
        private string Read_M2(string data_label)
        {
            ...
            int[] iData = new int[length];
            PLC_connect_DN20_2();
            iReturnCode = DN20_2.ReadDeviceRandom(data_label, iNumberOfData, out iData[0]);
            PLC_disconnect_DN20_2();
            if (iReturnCode == 0)
            {
                return iData[0].ToString();
            }
        ...
        private string Read_D2(string data_label)
        {
            ...
            short[] iData = new short[length];
            PLC_connect_DN20_2();
            iReturnCode = DN20_2.ReadDeviceBlock2(data_label, iNumberOfData, out iData[0]);
            PLC_disconnect_DN20_2();
            if (iReturnCode == 0)
            {
                return iData[0].ToString();
        ...
        public string Write_M2(string data_address, string value)
        {
            ...
            PLC_connect_DN20_2();
            iReturnCode = DN20_2.WriteDeviceRandom(data_address, iNumberOfData, ref iData[0]);
            PLC_disconnect_DN20_2();
            if (iReturnCode == 0)
            {
                return String.Format("0x{0:x8} [HEX]", iReturnCode);
        ...
        private string Write_D2(string data_address, string value)
        {
            ...
            PLC_connect_DN20_2();
            iReturnCode = DN20_2.WriteDeviceBlock2(data_address, iNumberOfData, ref iData[0]);
            PLC_disconnect_DN20_2();
            if (iReturnCode == 0)
            {
                return String.Format("0x{0:x8} [HEX]", iReturnCode);
